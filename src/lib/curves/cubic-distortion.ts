...
export function cubicDistortionCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float CubicDistortion(float x, float drive = 1) { return x - drive * x * x * x / 3; }",
      ...
      python:
        "def cubic_distortion(x, drive=1): drive = drive or 1; return x - drive * x ** 3 / 3",
      ...
      gdscript:
        "func cubic_distortion(x: float, drive: float = 1.0) -> float: return x - drive * x * x * x / 3.0",
      ...
      metal:
        "float cubicDistortion(float x, float drive) { drive = (drive == 0.0) ? 1.0 : drive; return x - drive * x * x * x / 3.0; }",
      opencl:
        "float cubicDistortion(float x, float drive) { return x - drive * x * x * x / 3.0f; }",
      unity:
        "public static float CubicDistortion(float x, float drive = 1) { return x - drive * x * x * x / 3; }",
      shadertoy: "return x - drive * x * x * x / 3.0;",
    },
    kernel,
