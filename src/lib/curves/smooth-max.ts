...
export function smoothMaxCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SmoothMax(float x, float k = 1) { k = k == 0 ? 1 : k; return MathF.Log(MathF.Exp(k * x) + MathF.Exp(k * (1 - x))) / k; }",
      ...
      python: "def smooth_max(x, k=1): k = k or 1; return math.log(math.exp(k * x) + math.exp(k * (1 - x))) / k",
      ...
      gdscript: "func smoothMax(x: float, k: float = 1.0) -> float: if k == 0: k = 1.0; return log(exp(k * x) + exp(k * (1.0 - x))) / k",
