...
export function softClipCurve(
  ...
  return {
    ...
    snippets: {
      equation:
        "y = (|t| < t) ? t : sign(t) * (t + (1-t) * tanh((|t|-t)/(1-t)))",
      ...
      csharp:
        "float SoftClip(float x, float t = 0.5f) { float a = Math.Abs(x); return a < t ? x : Math.Sign(x) * (t + (1 - t) * MathF.Tanh((a - t) / (1 - t))); }",
      ...
      python:
        "def soft_clip(x, t=0.5): t = t or 0.5; a = abs(x); return x if a < t else math.copysign(1, x) * (t + (1 - t) * math.tanh((a - t) / (1 - t)))",
      ...
      gdscript:
        "func softClip(x: float, t: float = 0.5) -> float: var a = abs(x); if a < t: return x; var s = -1.0 if x < 0 else 1.0; return s * (t + (1.0 - t) * tanh((a - t) / (1.0 - t)))",
      ...
      metal:
        "float softClip(float x, float t) { t = (t == 0.0) ? 0.5 : t; float a = abs(x); return a < t ? x : sign(x) * (t + (1.0 - t) * tanh((a - t) / (1.0 - t))); }",
      opencl:
        "float softClip(float x, float t) { if (t == 0.0f) t = 0.5f; float a = fabs(x); return a < t ? x : (x < 0.0f ? -1.0f : 1.0f) * (t + (1.0f - t) * tanhf((a - t) / (1.0f - t))); }",
      unity:
        "public static float SoftClip(float x, float t = 0.5f) { float a = Mathf.Abs(x); return a < t ? x : Mathf.Sign(x) * (t + (1 - t) * Mathf.Tanh((a - t) / (1 - t))); }",
    },
    kernel,
