...
export function svfCurve(
  params: SvfParams = defaultParams,
): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Svf(float x, float c = 0.3f, float r = 0.5f, float m = 0) { float f = 2 * MathF.Sin(MathF.PI * c); float q = 1 - r; float lp = 0, bp = 0; for (int i = 0; i < 10; i++) { float hp = x - lp - q * bp; bp = bp + f * hp; lp = lp + f * bp; } return m < 1 ? lp : m < 2 ? bp : hp; }",
      ...
      python: "def svf(x, c=0.3, r=0.5, m=0): f = 2 * math.sin(math.pi * c); q = 1 - r; lp = 0; bp = 0; for _ in range(10): hp = x - lp - q * bp; bp = bp + f * hp; lp = lp + f * bp; return lp if m < 1 else bp if m < 2 else hp",
      ...
      gdscript: "func svf(x: float, c: float = 0.3, r: float = 0.5, m: float = 0.0) -> float: var f = 2.0 * sin(PI * c); var q = 1.0 - r; var lp = 0.0; var bp = 0.0; for i in range(10): var hp = x - lp - q * bp; bp = bp + f * hp; lp = lp + f * bp; return lp if m < 1 else bp if m < 2 else hp",
