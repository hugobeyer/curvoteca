...
export function wavefolderCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Wavefolder(float x, float fold = 2) { float v = x * fold; return Math.Abs((v + 1) % 2 - 1) * 2 - 1; }",
      ...
      python:
        "def wavefolder(x, fold=2): fold = fold or 2; v = x * fold; return abs(((v + 1) % 2 - 1)) * 2 - 1",
      ...
      gdscript:
        "func wavefolder(x: float, fold: float = 2.0) -> float: var v = x * fold; return abs(fmod(v + 1.0, 2.0) - 1.0) * 2.0 - 1.0",
      ...
      metal:
        "float wavefolder(float x, float fold) { fold = (fold == 0.0) ? 2.0 : fold; float v = x * fold; return abs(mod(v + 1.0, 2.0) - 1.0) * 2.0 - 1.0; }",
      opencl:
        "float wavefolder(float x, float fold) { if (fold == 0.0f) fold = 2.0f; float v = x * fold; return fabs(fmod(v + 1.0f, 2.0f) - 1.0f) * 2.0f - 1.0f; }",
      unity:
        "public static float Wavefolder(float x, float fold = 2) { float v = x * fold; return Mathf.Abs((v + 1) % 2 - 1) * 2 - 1; }",
    },
    kernel,
