...
export function gaussianCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Gaussian(float x, float mu = 0.5f, float sigma = 0.25f) { float z = (x - mu) / sigma; return MathF.Exp(-0.5f * z * z); }",
      ...
      python:
        "def gaussian(x, mu=0.5, sigma=0.25): z = (x - mu) / sigma; return math.exp(-0.5 * z * z)",
      ...
      gdscript:
        "func gaussian(x: float, mu: float = 0.5, sigma: float = 0.25) -> float: var z = (x - mu) / sigma; return exp(-0.5 * z * z)",
      ...
      metal:
        "float gaussian(float x, float mu, float sigma) { float z = (x - mu) / sigma; return exp(-0.5 * z * z); }",
      opencl:
        "float gaussian(float x, float mu, float sigma) { float z = (x - mu) / sigma; return exp(-0.5f * z * z); }",
      unity:
        "public static float Gaussian(float x, float mu, float sigma) { float z = (x - mu) / sigma; return Mathf.Exp(-0.5f * z * z); }",
      matlab: "y = @(x, mu, sigma) exp(-0.5 * ((x - mu) / sigma) ^ 2);",
      excel: "=EXP(-0.5*((A1-A2)/A3)^2)",
      desmos: "y=e^{-\\frac{1}{2}\\left(\\frac{x-\\mu}{\\sigma}\\right)^{2}}",
    },
    kernel,
