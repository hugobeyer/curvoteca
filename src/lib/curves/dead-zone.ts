...
export function deadZoneCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float DeadZone(float x, float d = 0.2f) { return MathF.Abs(x) < d ? 0.0f : MathF.Sign(x) * (MathF.Abs(x) - d) / (1.0f - d); }",
      ...
      python: "def deadZone(x, d=0.2): return 0 if abs(x) < d else math.copysign(1, x) * (abs(x) - d) / (1 - d)",
      ...
      gdscript: "func dead_zone(x: float, d: float = 0.2) -> float: return 0.0 if abs(x) < d else sign(x) * (abs(x) - d) / (1.0 - d)",
