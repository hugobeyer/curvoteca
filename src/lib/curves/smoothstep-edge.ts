...
export function smoothstepEdgeCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SmoothstepEdge(float x, float a, float b) { if (x <= a) return 0; if (x >= b) return 1; float t = (x - a) / (b - a); return t * t * (3 - 2 * t); }",
      ...
      python: "def smoothstep_edge(x, a, b): if x <= a: return 0; if x >= b: return 1; t = (x - a) / (b - a); return t * t * (3 - 2 * t)",
      ...
      gdscript: "func smoothstepEdge(x: float, a: float = 0.2, b: float = 0.8) -> float: var t = clamp((x - a) / (b - a), 0.0, 1.0); return t * t * (3.0 - 2.0 * t)",
