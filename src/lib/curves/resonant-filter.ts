...
export function resonantFilterCurve(
  ...
  const kernel: CurveKernel = resonantFilterKernel(
    params.cutoff,
    params.resonance,
  );
  return {
    ...
    snippets: {
      ...
      csharp:
        "float ResonantFilter(float x, float fc = 0.3f, float R = 0.5f) { float ratio = x / fc; return 1 / MathF.Sqrt(MathF.Pow(1 - ratio * ratio, 2) + MathF.Pow(2 * R * ratio, 2)); }",
      ...
      python:
        "def resonant_filter(x, fc=0.3, R=0.5): fc = fc or 0.3; R = R or 0.5; ratio = x / fc; return 1 / math.sqrt((1 - ratio * ratio) ** 2 + (2 * R * ratio) ** 2)",
      ...
      gdscript:
        "func resonantFilter(x: float, fc: float = 0.3, R: float = 0.5) -> float: var ratio = x / fc; return 1.0 / sqrt(pow(1.0 - ratio * ratio, 2.0) + pow(2.0 * R * ratio, 2.0))",
      ...
      metal:
        "float resonantFilter(float x, float fc, float R) { fc = (fc == 0.0) ? 0.3 : fc; R = (R == 0.0) ? 0.5 : R; float ratio = x / fc; return 1.0 / sqrt(pow(1.0 - ratio * ratio, 2.0) + pow(2.0 * R * ratio, 2.0)); }",
      opencl:
        "float resonantFilter(float x, float fc, float R) { float ratio = x / fc; return 1.0f / sqrt(pow(1.0f - ratio * ratio, 2.0f) + pow(2.0f * R * ratio, 2.0f)); }",
      unity:
        "public static float ResonantFilter(float x, float fc = 0.3f, float R = 0.5f) { float ratio = x / fc; return 1 / Mathf.Sqrt(Mathf.Pow(1 - ratio * ratio, 2) + Mathf.Pow(2 * R * ratio, 2)); }",
    },
    kernel,
