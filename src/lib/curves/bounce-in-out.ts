...
export function bounceInOutCurve(
  ...
  return {
    ...
    formula: "y = (x < 0.5) ? 0.5 * (1 - bounce_out(1 - 2x)) : 0.5 * (1 + bounce_out(2x - 1))",
    continuity: "C0",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      equation: "y = (x < 0.5) ? 0.5 * (1 - bounce_out(1 - 2x)) : 0.5 * (1 + bounce_out(2x - 1))",
      ...
      python: "def bounce_in_out(x, n=4, k=2): n = max(1, n); k = k or 2; if x <= 0: return 0; if x >= 1: return 1; if x < 0.5: return 0.5 * (1 - bounce_out(1 - 2 * x, n, k)); return 0.5 * (1 + bounce_out(2 * x - 1, n, k)); def bounce_out(t, n, k): if t <= 0: return 0; if t >= 1: return 1; env = math.pow(1 - t, k); osc = (1 + math.cos(2 * math.pi * n * t)) * 0.5; return 1 - env * osc",
