...
export function overshootSettleCurve(
  ...
  return {
    ...
    formula:
      "y = x + overshoot * x * (1-x) * e^(-settle * x)",
    continuity: "C1",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      ...
      csharp: "float OvershootSettle(float x, float o = 0.2f, float s = 0.5f) { return x + o * x * (1 - x) * MathF.Exp(-s * x); }",
      ...
      python: "def overshoot_settle(x, o=0.2, s=0.5): o = o if o >= -10 else 0.2; s = s or 0.5; return x + o * x * (1 - x) * math.exp(-s * x)",
      ...
      gdscript: "func overshootSettle(x: float, o: float = 0.2, s: float = 0.5) -> float: return x + o * x * (1 - x) * exp(-s * x)",
