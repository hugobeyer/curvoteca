...
export function inverseLerpCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float InverseLerp(float x, float lo = 0.2f, float hi = 0.8f) { return (x - lo) / (hi - lo); }",
      ...
      python: "def inverse_lerp(x, lo=0.2, hi=0.8): return (x - lo) / (hi - lo)",
      ...
      gdscript: "func inverse_lerp(x: float, lo: float = 0.2, hi: float = 0.8) -> float: return (x - lo) / (hi - lo)",
