...
export function fresnelCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Fresnel(float x, float p = 5, float b = 0) { return b + (1 - b) * MathF.Pow(1 - x, p); }",
      ...
      python: "def fresnel(x, p=5, b=0): p = p if p >= 0.01 else 5; return b + (1 - b) * math.pow(1 - x, p)",
      ...
      gdscript: "func fresnel(x: float, p: float = 5.0, b: float = 0.0) -> float: return b + (1 - b) * pow(1 - x, p)",
