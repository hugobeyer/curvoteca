import { DEFAULT_SAMPLING, quarticInOutKernel, type CurveKernel } from "../curveMath";
...
export function quarticInOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float QuarticInOut(float x) { return x < 0.5f ? 8.0f * x * x * x * x : 1.0f - MathF.Pow(-2.0f * x + 2.0f, 4.0f) / 2.0f; }",
      ...
      python: "def quartic_in_out(x): return 8 * x * x * x * x if x < 0.5 else 1 - math.pow(-2 * x + 2, 4) / 2",
      ...
      gdscript: "func quarticInOut(x: float) -> float: return 8 * x * x * x * x if x < 0.5 else 1 - pow(-2 * x + 2, 4) / 2",
