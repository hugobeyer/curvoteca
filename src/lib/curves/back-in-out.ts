...
export function backInOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float BackInOut(float x, float s = 1.70158f) { float sOut = 2 * s; if (x < 0.5) { float u = 2 * x; return 0.5f * u * u * ((sOut + 1) * u - sOut); } float u = 2 * x - 2; return 0.5f * (u * u * ((sOut + 1) * u + sOut) + 2); }",
      ...
      python: "def back_in_out(x, s=1.70158): s_out = 2 * s; u = 2 * x if x < 0.5 else 2 * x - 2; return 0.5 * u * u * ((s_out + 1) * u - s_out) if x < 0.5 else 0.5 * (u * u * ((s_out + 1) * u + s_out) + 2)",
      ...
      gdscript: "func backInOut(x: float, s: float = 1.70158) -> float: var sOut = 2 * s; if x < 0.5: var u = 2 * x; return 0.5 * u * u * ((sOut + 1) * u - sOut); var u = 2 * x - 2; return 0.5 * (u * u * ((sOut + 1) * u + sOut) + 2)",
