import { DEFAULT_SAMPLING, arKernel, type CurveKernel } from "../curveMath";
import type { CurveDefinition } from "../../data/curves";

export const defaultParams = { attack: 0.3, release: 0.4 } as const;
export type ArParams = { attack: number; release: number };

export function arCurve(params: ArParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Ar(float x, float a = 0.3f, float r = 0.4f) { return x < a ? x / a : MathF.Exp(-(x - a) / r); }",
      ...
      python:
        "def ar(x, a=0.3, r=0.4): return x / a if x < a else math.exp(-(x - a) / r)",
      ...
      gdscript:
        "func ar(x: float, a: float = 0.3, r: float = 0.4) -> float: return x / a if x < a else exp(-(x - a) / r)",
      ...
      metal:
        "float ar(float x, float a, float r) { a = (a < 0.01) ? 0.3 : a; r = (r < 0.01) ? 0.4 : r; return x < a ? x / a : exp(-(x - a) / r); }",
      opencl:
        "float ar(float x, float a, float r) { return x < a ? x / a : expf(-(x - a) / r); }",
      unity:
        "public static float Ar(float x, float a = 0.3f, float r = 0.4f) { return x < a ? x / a : MathF.Exp(-(x - a) / r); }",
    },
    kernel,
