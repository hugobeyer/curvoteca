...
export function cosineEaseCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float CosineEase(float x) { return 1 - MathF.Cos(x * MathF.PI / 2); }",
      ...
      gdscript:
        "func cosine_ease(x: float) -> float: return 1.0 - cos(x * PI / 2.0)",
      ...
      metal: "float cosineEase(float x) { return 1.0 - cos(x * 1.57079633); }",
      opencl:
        "float cosineEase(float x) { return 1.0f - cosf(x * 3.14159265f / 2.0f); }",
      unity:
        "public static float CosineEase(float x) { return 1 - MathF.Cos(x * MathF.PI / 2); }",
      shadertoy: "return 1.0 - cos(x * 1.57079633);",
      svelte: "export const cosineEase = (x) => 1 - Math.cos(x * Math.PI / 2);",
