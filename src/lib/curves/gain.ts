...
export function gainCurve(params: GainParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Gain(float x, float b) { return x < 0.5f ? Bias(2 * x, b) / 2 : 1 - Bias(2 - 2 * x, b) / 2; }",
      ...
      python: "def gain(x, b): return bias(2 * x, b) / 2 if x < 0.5 else 1 - bias(2 - 2 * x, b) / 2",
      ...
      gdscript: "func gain(x: float, b: float) -> float: return bias(2 * x, b) / 2 if x < 0.5 else 1 - bias(2 - 2 * x, b) / 2",
