...
export function remapCurve(
  ...
  return {
    ...
    formula:
      "y = loOut + (t - loIn) * (hiOut - loOut) / (hiIn - loIn)",
    continuity: "C3+",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      ...
      csharp: "float Remap(float x, float loIn = 0.2f, float hiIn = 0.8f, float loOut = 0, float hiOut = 1) { return loOut + (x - loIn) * (hiOut - loOut) / (hiIn - loIn); }",
      ...
      python: "def remap(x, lo_in=0.2, hi_in=0.8, lo_out=0, hi_out=1): return lo_out + (x - lo_in) * (hi_out - lo_out) / (hi_in - lo_in)",
      ...
      gdscript: "func remap(x: float, loIn: float = 0.2, hiIn: float = 0.8, loOut: float = 0.0, hiOut: float = 1.0) -> float: return loOut + (x - loIn) * (hiOut - loOut) / (hiIn - loIn)",
