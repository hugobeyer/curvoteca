...
export function cosineOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float CosineOut(float x) { return 1 - MathF.Sin((1 - x) * MathF.PI / 2); }",
      ...
      gdscript:
        "func cosine_out(x: float) -> float: return 1.0 - sin((1.0 - x) * PI / 2.0)",
      ...
      metal:
        "float cosineOut(float x) { return 1.0 - sin((1.0 - x) * 1.57079633); }",
      opencl:
        "float cosineOut(float x) { return 1.0f - sinf((1.0f - x) * 3.14159265f / 2.0f); }",
      unity:
        "public static float CosineOut(float x) { return 1 - MathF.Sin((1 - x) * MathF.PI / 2); }",
      shadertoy: "return 1.0 - sin((1.0 - x) * 1.57079633);",
      svelte:
        "export const cosineOut = (x) => 1 - Math.sin((1 - x) * Math.PI / 2);",
      matlab: "y = @(x) 1 - sin((1 - x) * pi / 2);",
      excel: "=1-SIN((1-A1)*PI()/2)",
      desmos: "y = 1 - \\sin((1 - x) \\pi / 2)",
    },
    kernel,
