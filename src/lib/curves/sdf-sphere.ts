import { DEFAULT_SAMPLING, sdfSphereKernel, type CurveKernel } from "../curveMath";
...
export function sdfSphereCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SdfSphere(float x, float r = 0.3f, float cx = 0.5f) { return Math.Abs(x - cx) - r; }",
      ...
      python: "def sdf_sphere(x, r=0.3, cx=0.5): r = r or 0.3; cx = cx or 0.5; return abs(x - cx) - r",
      ...
      gdscript: "func sdfSphere(x: float, r: float = 0.3, cx: float = 0.5) -> float: return abs(x - cx) - r",
