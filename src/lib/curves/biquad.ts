...
export function biquadCurve(
  ...
  const kernel: CurveKernel = biquadKernel(
    params.a0,
    params.a1,
    params.a2,
    params.b1,
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Biquad(float x, float a0 = 1, float a1 = 0, float a2 = 0, float b1 = 0, float b2 = 0) { float w = MathF.PI * x; float c = MathF.Cos(w); float num = a0 + a1 * c + a2 * MathF.Cos(2 * w); float den = 1 + b1 * c + b2 * MathF.Cos(2 * w); return MathF.Sqrt((num * num) / (den * den)); }",
      ...
      python:
        "def biquad(x, a0=1, a1=0, a2=0, b1=0, b2=0): a0 = a0 or 1; w = math.pi * x; c = math.cos(w); num = a0 + a1 * c + a2 * math.cos(2 * w); den = 1 + b1 * c + b2 * math.cos(2 * w); return math.sqrt((num * num) / (den * den))",
      ...
      gdscript:
        "func biquad(x: float, a0: float = 1.0, a1: float = 0.0, a2: float = 0.0, b1: float = 0.0, b2: float = 0.0) -> float: var w = PI * x; var c = cos(w); var num = a0 + a1 * c + a2 * cos(2 * w); var den = 1 + b1 * c + b2 * cos(2 * w); return sqrt((num * num) / (den * den))",
      ...
      metal:
        "float biquad(float x, float a0, float a1, float a2, float b1, float b2) { a0 = (a0 == 0.0) ? 1.0 : a0; float w = 3.14159265 * x; float c = cos(w); float num = a0 + a1 * c + a2 * cos(2.0 * w); float den = 1.0 + b1 * c + b2 * cos(2.0 * w); return sqrt((num * num) / (den * den)); }",
      opencl:
        "float biquad(float x, float a0, float a1, float a2, float b1, float b2) { a0 = (a0 == 0.0) ? 1.0 : a0; float w = 3.14159265f * x; float c = cos(w); float num = a0 + a1 * c + a2 * cos(2.0f * w); float den = 1.0f + b1 * c + b2 * cos(2.0f * w); return sqrt((num * num) / (den * den)); }",
      unity:
        "public static float Biquad(float x, float a0 = 1, float a1 = 0, float a2 = 0, float b1 = 0, float b2 = 0) { float w = MathF.PI * x; float c = MathF.Cos(w); float num = a0 + a1 * c + a2 * MathF.Cos(2 * w); float den = 1 + b1 * c + b2 * MathF.Cos(2 * w); return MathF.Sqrt((num * num) / (den * den)); }",
    },
    kernel,
