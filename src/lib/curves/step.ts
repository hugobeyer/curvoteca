...
export function stepCurve(params: StepParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Step(float x, float edge = 0.5f) { return x < edge ? 0 : 1; }",
      ...
      gdscript:
        "func step(x: float, edge: float = 0.5) -> float: return 0.0 if x < edge else 1.0",
      ...
      metal: "float step(float x, float edge) { return x < edge ? 0.0 : 1.0; }",
      opencl:
        "float step(float x, float edge) { return x < edge ? 0.0f : 1.0f; }",
      unity:
        "public static float Step(float x, float edge = 0.5f) { return x < edge ? 0 : 1; }",
      matlab: "y = @(x, edge) double(x >= edge);",
    },
    kernel,
