...
export function quinticInOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float QuinticInOut(float x) { return x < 0.5f ? 16.0f * x * x * x * x * x : 1.0f - MathF.Pow(-2.0f * x + 2.0f, 5.0f) / 2.0f; }",
      ...
      python:
        "def quintic_in_out(x): return 16 * x * x * x * x * x if x < 0.5 else 1 - math.pow(-2 * x + 2, 5) / 2",
      ...
      gdscript:
        "func quinticInOut(x: float) -> float: return 16 * x * x * x * x * x if x < 0.5 else 1 - pow(-2 * x + 2, 5) / 2",
      ...
      metal:
        "float quinticInOut(float x) { return x < 0.5 ? 16.0 * x * x * x * x * x : 1.0 - pow(-2.0 * x + 2.0, 5.0) / 2.0; }",
      opencl:
        "float quinticInOut(float x) { return x < 0.5f ? 16.0f * x * x * x * x * x : 1.0f - pow(-2.0f * x + 2.0f, 5.0f) / 2.0f; }",
      unity:
        "public static float QuinticInOut(float x) { return x < 0.5f ? 16.0f * x * x * x * x * x : 1.0f - Mathf.Pow(-2.0f * x + 2.0f, 5.0f) / 2.0f; }",
      shadertoy:
        "return x < 0.5 ? 16.0 * x * x * x * x * x : 1.0 - pow(-2.0 * x + 2.0, 5.0) / 2.0;",
      svelte:
        "export const quinticInOut = (x) => x < 0.5 ? 16 * x * x * x * x * x : 1 - Math.pow(-2 * x + 2, 5) / 2;",
      matlab:
        "y = @(x) x^5 * 16 * (x < 0.5) + (1 - (-2*x + 2)^5 / 2) * (x >= 0.5);",
      excel: "=IF(A1<0.5, 16*A1^5, 1-(-2*A1+2)^5/2)",
      desmos:
        "y = \\left\\{x < 0.5: 16 x^{5}, x \\geq 0.5: 1 - \\left(-2 x + 2\\right)^{5} / 2\\right\\}",
    },
    kernel,
