...
export function sawtoothWaveCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SawtoothWave(float x, float freq = 4) { return ((x * freq) % 1) * 2 - 1; }",
      ...
      gdscript:
        "func sawtoothWave(x: float, freq: float = 4.0) -> float: return fmod(x * freq, 1.0) * 2.0 - 1.0",
      ...
      metal:
        "float sawtoothWave(float x, float freq) { return fract(x * freq) * 2.0 - 1.0; }",
      opencl:
        "float sawtoothWave(float x, float freq) { return fmod(x * freq, 1.0f) * 2.0f - 1.0f; }",
      unity:
        "public static float SawtoothWave(float x, float freq = 4) { return ((x * freq) % 1) * 2 - 1; }",
      shadertoy: "return fract(x * freq) * 2.0 - 1.0;",
      matlab: "y = @(x, freq) mod(x * freq, 1) * 2 - 1;",
    },
    kernel,
