...
export function sineInCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SineIn(float x) { return 1.0f - MathF.Cos(x * 1.57079633f); }",
      ...
      gdscript:
        "func sineIn(x: float) -> float: return 1.0 - cos(x * PI / 2.0)",
      ...
      metal: "float sineIn(float x) { return 1.0 - cos(x * 1.57079633); }",
      opencl: "float sineIn(float x) { return 1.0f - cos(x * 1.57079633f); }",
      unity:
        "public static float SineIn(float x) { return 1.0f - Mathf.Cos(x * 1.57079633f); }",
      shadertoy: "return 1.0 - cos(x * 1.57079633);",
      svelte: "export const sineIn = (x) => 1 - Math.cos(x * Math.PI / 2);",
      matlab: "y = @(x) 1 - cos(x * pi / 2);",
      excel: "=1-COS(A1*PI()/2)",
      desmos: "y = 1 - \\cos\\left(x \\pi / 2\\right)",
    },
    kernel,
