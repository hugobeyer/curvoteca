...
export function whiteNoise1dCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float WhiteNoise1d(float x, float seed = 1337) { /* hash(x, seed) * 2 - 1 */ return WhiteNoise1dImpl(x, seed); }",
      ...
      python: "def white_noise_1d(x, seed=1337): # hash(x, seed) * 2 - 1; return white_noise_1d_impl(x, seed)",
      ...
      gdscript: "func whiteNoise1d(x: float, seed: float = 1337.0) -> float: # hash(x, seed) * 2 - 1; return whiteNoise1dImpl(x, seed)",
