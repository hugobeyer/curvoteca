...
export function sdfTorusCurve(
  ...
  const kernel: CurveKernel = sdfTorusKernel(params.majorR, params.minorR, params.cx);
  return {
    id: "sdf-torus",
    name: "SDF Torus",
    aliases: [],
    ...
    domain: [0, 1],
    range: [-1, 0.5],
    tags: ["sdf", "signed-distance", "geometry", "primitive", "raymarching", "torus"],
    useCases: [
    ...
    snippets: {
      ...
      csharp: "float SdfTorus(float x, float R = 0.3f, float r = 0.1f, float cx = 0.5f) { float d = Math.Abs(x - cx); return Math.Abs(MathF.Sqrt(MathF.Pow(d - R, 2)) - r); }",
      ...
      python: "def sdf_torus(x, R=0.3, r=0.1, cx=0.5): R = R or 0.3; r = r or 0.1; cx = cx or 0.5; d = abs(x - cx); return abs(math.sqrt((d - R) ** 2) - r)",
      ...
      gdscript: "func sdfTorus(x: float, R: float = 0.3, r: float = 0.1, cx: float = 0.5) -> float: var d = abs(x - cx); return abs(sqrt(pow(d - R, 2.0)) - r)",
