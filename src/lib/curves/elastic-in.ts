...
export function elasticInCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float ElasticIn(float x, float a = 1, float p = 0.3f) { a = Math.Max(1, a); p = p == 0 ? 0.3f : p; if (x == 0 || x == 1) return x; float s = (p / (2 * MathF.PI)) * MathF.Asin(1 / a); return -(a * MathF.Pow(2, 10 * x - 10) * MathF.Sin(((x - 1 - s) * 2 * MathF.PI) / p)); }",
      ...
      python: "def elastic_in(x, a=1, p=0.3): a = max(1, a); p = p or 0.3; if x == 0 or x == 1: return x; s = (p / (2 * math.pi)) * math.asin(1 / a); return -(a * math.pow(2, 10 * x - 10) * math.sin(((x - 1 - s) * 2 * math.pi) / p))",
      ...
      gdscript: "func elastic_in(x: float, a: float = 1.0, p: float = 0.3) -> float: if x == 0.0 or x == 1.0: return x; var s = (p / (2.0 * PI)) * asin(1.0 / a); return -(a * pow(2.0, 10.0 * x - 10.0) * sin(((x - 1.0 - s) * 2.0 * PI) / p))",
