...
export type VoronoiJitterParams = { seed: number; cells: number; jitter: number };

export function voronoiJitterCurve(
  params: VoronoiJitterParams = defaultParams,
): CurveDefinition {
  const kernel: CurveKernel = voronoiJitterKernel(params.seed, params.cells, params.jitter);
  return {
    id: "voronoi-jitter",
    name: "Voronoi Jitter",
    aliases: [],
    ...
    snippets: {
      ...
      csharp: "float VoronoiJitter(float x, int seed = 1337, int cells = 6, float jitter = 0.5f) { return 0; }",
      ...
      gdscript: "func voronoiJitter(x: float, seed: int = 1337, cells: int = 6, jitter: float = 0.5) -> float: return 0.0",
