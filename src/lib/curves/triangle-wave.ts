...
export function triangleWaveCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float TriangleWave(float x, float freq = 4) { float f = (x * freq) % 1; return 2 * Math.Abs(2 * f - 1) - 1; }",
      ...
      python:
        "def triangle_wave(x, freq=4): f = (x * freq) % 1; return 2 * abs(2 * f - 1) - 1",
      ...
      gdscript:
        "func triangleWave(x: float, freq: float = 4.0) -> float: if freq < 0.01: freq = 4.0; var f = fmod(x * freq, 1.0); return 2.0 * abs(2.0 * f - 1.0) - 1.0",
      ...
      metal:
        "float triangleWave(float x, float freq) { float f = fract(x * freq); return 2.0 * abs(2.0 * f - 1.0) - 1.0; }",
      opencl:
        "float triangleWave(float x, float freq) { if (freq < 0.01f) freq = 4.0f; float f = fmod(x * freq, 1.0f); return 2.0f * fabs(2.0f * f - 1.0f) - 1.0f; }",
      unity:
        "public static float TriangleWave(float x, float freq = 4) { float f = (x * freq) % 1; return 2 * Mathf.Abs(2 * f - 1) - 1; }",
      shadertoy: "return 2.0 * abs(2.0 * fract(x * freq) - 1.0) - 1.0;",
      matlab: "y = @(x, freq) 2 * abs(2 * mod(x * freq, 1) - 1) - 1;",
    },
    kernel,
