...
export function powerCurve(
  ...
  return {
    ...
    snippets: {
      ...
      gdscript:
        "func power(x: float, p: float = 2.0) -> float: return pow(x, p)",
      ...
      metal: "float power(float x, float p) { return pow(x, p); }",
      opencl: "float power(float x, float p) { return pow(x, p); }",
      unity:
        "public static float Power(float x, float p) { return Mathf.Pow(x, p); }",
      matlab: "y = @(x, p) x ^ p;",
      excel: "=POWER(A1,A2)",
      desmos: "y=x^{p}",
    },
