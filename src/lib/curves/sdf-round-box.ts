...
export function sdfRoundBoxCurve(
  ...
  const kernel: CurveKernel = sdfRoundBoxKernel(
    params.width,
    params.cx,
    params.r,
  );
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SdfRoundBox(float x, float w = 0.3f, float cx = 0.5f, float r = 0.05f) { return Math.Max(Math.Abs(x - cx) - w / 2 + r, 0) - r; }",
      ...
      python:
        "def sdf_round_box(x, w=0.3, cx=0.5, r=0.05): w = w or 0.3; cx = cx or 0.5; r = r or 0.05; return max(abs(x - cx) - w / 2 + r, 0) - r",
      ...
      gdscript:
        "func sdfRoundBox(x: float, w: float = 0.3, cx: float = 0.5, r: float = 0.05) -> float: return max(abs(x - cx) - w / 2.0 + r, 0.0) - r",
      ...
      metal:
        "float sdfRoundBox(float x, float w, float cx, float r) { w = (w == 0.0) ? 0.3 : w; cx = (cx == 0.0) ? 0.5 : cx; r = (r == 0.0) ? 0.05 : r; return max(abs(x - cx) - w / 2.0 + r, 0.0) - r; }",
      opencl:
        "float sdfRoundBox(float x, float w, float cx, float r) { return fmax(fabs(x - cx) - w / 2.0f + r, 0.0f) - r; }",
      unity:
        "public static float SdfRoundBox(float x, float w = 0.3f, float cx = 0.5f, float r = 0.05f) { return Mathf.Max(Mathf.Abs(x - cx) - w / 2 + r, 0) - r; }",
    },
    kernel,
