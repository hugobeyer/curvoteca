...
export const defaultParams = { modIndex: 1, modFreq: 3, carrierFreq: 5 } as const;
...
export function fmFeedbackCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float FmFeedback(float x, float mi = 1, float fm = 3, float fc = 5) { return MathF.Sin(2 * MathF.PI * fc * x + mi * MathF.Sin(2 * MathF.PI * fm * x)); }",
      ...
      python: "def fm_feedback(x, mi=1, fm=3, fc=5): return math.sin(2 * math.pi * fc * x + mi * math.sin(2 * math.pi * fm * x))",
      ...
      gdscript: "func fm_feedback(x: float, mi: float = 1.0, fm: float = 3.0, fc: float = 5.0) -> float: return sin(2 * PI * fc * x + mi * sin(2 * PI * fm * x))",
