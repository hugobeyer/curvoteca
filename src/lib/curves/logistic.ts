...
export function logisticCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Logistic(float x, float k, float x0) { return 1.0f / (1.0f + MathF.Exp(-k * (x - x0))); }",
      ...
      python: "def logistic(x, k, x0): return 1 / (1 + math.exp(-k * (x - x0)))",
      ...
      gdscript: "func logistic(x: float, k: float, x0: float) -> float: return 1 / (1 + exp(-k * (x - x0)))",
