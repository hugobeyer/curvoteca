...
export function logisticMapCurve(
  ...
  const kernel: CurveKernel = logisticMapKernel(
    params.r,
    params.iterations,
  );
  return {
    ...
    snippets: {
      ...
      csharp: "float LogisticMap(float x, float r = 3.8f, int it = 100) { float v = x; for (int i = 0; i < it; i++) { v = r * v * (1 - v); } return v; }",
      ...
      python: "def logistic_map(x, r=3.8, it=100): v = x; for _ in range(it): v = r * v * (1 - v); return v",
      ...
      gdscript: "func logistic_map(x: float, r: float = 3.8, it: int = 100) -> float: var v = x; for i in range(it): v = r * v * (1 - v); return v",
