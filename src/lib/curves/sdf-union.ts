...
export function sdfUnionCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SdfUnion(float x, float r1 = 0.3f, float c1 = 0.3f, float r2 = 0.3f, float c2 = 0.7f) { float d1 = MathF.Abs(x - c1) - r1; float d2 = MathF.Abs(x - c2) - r2; return MathF.Min(d1, d2); }",
      ...
      python:
        "def sdf_union(x, r1=0.3, c1=0.3, r2=0.3, c2=0.7): d1 = abs(x - c1) - r1; d2 = abs(x - c2) - r2; return min(d1, d2)",
      ...
      gdscript:
        "func sdfUnion(x: float, r1: float = 0.3, c1: float = 0.3, r2: float = 0.3, c2: float = 0.7) -> float: var d1 = abs(x - c1) - r1; var d2 = abs(x - c2) - r2; return min(d1, d2)",
      ...
      metal:
        "float sdfUnion(float x, float r1, float c1, float r2, float c2) { r1 = (r1 == 0.0) ? 0.3 : r1; c1 = (c1 == 0.0) ? 0.3 : c1; r2 = (r2 == 0.0) ? 0.3 : r2; c2 = (c2 == 0.0) ? 0.7 : c2; float d1 = abs(x - c1) - r1; float d2 = abs(x - c2) - r2; return min(d1, d2); }",
      opencl:
        "float sdfUnion(float x, float r1, float c1, float r2, float c2) { float d1 = fabs(x - c1) - r1; float d2 = fabs(x - c2) - r2; return fmin(d1, d2); }",
      unity:
        "public static float SdfUnion(float x, float r1 = 0.3f, float c1 = 0.3f, float r2 = 0.3f, float c2 = 0.7f) { float d1 = Mathf.Abs(x - c1) - r1; float d2 = Mathf.Abs(x - c2) - r2; return Mathf.Min(d1, d2); }",
    },
    kernel,
