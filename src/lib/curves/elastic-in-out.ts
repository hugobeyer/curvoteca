...
export function elasticInOutCurve(
  ...
  const kernel: CurveKernel = elasticInOutKernel(
    params.amplitude,
    params.period,
  );
  return {
    ...
    formula:
      "y = (u < 0) ? -0.5 * a * 2^(10u) * sin((u-s)*2pi/p) : 0.5 * a * 2^(-10u) * sin((u-s)*2pi/p) + 1",
    continuity: "C0",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      ...
      csharp:
        "float ElasticInOut(float x, float a = 1, float p = 0.3f) { a = Math.Max(1, a); p = p == 0 ? 0.3f : p; if (x == 0 || x == 1) return x; float u = 2 * x - 1; float s = (p / (2 * MathF.PI)) * MathF.Asin(1 / a); if (u < 0) return -0.5f * a * MathF.Pow(2, 10 * u) * MathF.Sin(((u - s) * 2 * MathF.PI) / p); return 0.5f * a * MathF.Pow(2, -10 * u) * MathF.Sin(((u - s) * 2 * MathF.PI) / p) + 1; }",
      ...
      python:
        "def elastic_in_out(x, a=1, p=0.3): a = max(1, a); p = p or 0.3; if x == 0 or x == 1: return x; u = 2 * x - 1; s = (p / (2 * math.pi)) * math.asin(1 / a); if u < 0: return -0.5 * a * math.pow(2, 10 * u) * math.sin(((u - s) * 2 * math.pi) / p); return 0.5 * a * math.pow(2, -10 * u) * math.sin(((u - s) * 2 * math.pi) / p) + 1",
      ...
      gdscript:
        "func elastic_in_out(x: float, a: float = 1.0, p: float = 0.3) -> float: if x == 0.0 or x == 1.0: return x; var u = 2.0 * x - 1.0; var s = (p / (2.0 * PI)) * asin(1.0 / a); if u < 0.0: return -0.5 * a * pow(2.0, 10.0 * u) * sin(((u - s) * 2.0 * PI) / p); return 0.5 * a * pow(2.0, -10.0 * u) * sin(((u - s) * 2.0 * PI) / p) + 1.0",
      ...
      metal:
        "float elasticInOut(float x, float a, float p) { a = (a < 1.0) ? 1.0 : a; p = (p == 0.0) ? 0.3 : p; if (x == 0.0 || x == 1.0) return x; float u = 2.0 * x - 1.0; float s = (p / (2.0 * 1.57079633)) * asin(1.0 / a); if (u < 0.0) return -0.5 * a * pow(2.0, 10.0 * u) * sin(((u - s) * 2.0 * 1.57079633) / p); return 0.5 * a * pow(2.0, -10.0 * u) * sin(((u - s) * 2.0 * 1.57079633) / p) + 1.0; }",
      opencl:
        "float elasticInOut(float x, float a, float p) { a = (a < 1.0f) ? 1.0f : a; p = (p == 0.0f) ? 0.3f : p; if (x == 0.0f || x == 1.0f) return x; float u = 2.0f * x - 1.0f; float s = (p / (2.0f * 1.57079633f)) * asin(1.0f / a); if (u < 0.0f) return -0.5f * a * pow(2.0f, 10.0f * u) * sin(((u - s) * 2.0f * 1.57079633f) / p); return 0.5f * a * pow(2.0f, -10.0f * u) * sin(((u - s) * 2.0f * 1.57079633f) / p) + 1.0f; }",
      unity:
        "public static float ElasticInOut(float x, float a, float p) { a = Mathf.Max(1.0f, a); p = p == 0.0f ? 0.3f : p; if (x == 0.0f || x == 1.0f) return x; float u = 2.0f * x - 1.0f; float s = (p / (2.0f * Mathf.PI)) * Mathf.Asin(1.0f / a); if (u < 0.0f) return -0.5f * a * Mathf.Pow(2.0f, 10.0f * u) * Mathf.Sin(((u - s) * 2.0f * Mathf.PI) / p); return 0.5f * a * Mathf.Pow(2.0f, -10.0f * u) * Mathf.Sin(((u - s) * 2.0f * Mathf.PI) / p) + 1.0f; }",
      shadertoy:
        "float u = 2.0 * x - 1.0; float s = (p / (2.0 * 1.57079633)) * asin(1.0 / a); if (u < 0.0) return -0.5 * a * pow(2.0, 10.0 * u) * sin(((u - s) * 2.0 * 1.57079633) / p); return 0.5 * a * pow(2.0, -10.0 * u) * sin(((u - s) * 2.0 * 1.57079633) / p) + 1.0;",
      svelte:
        "export const elasticInOut = (x, a, p) => { a = a == null ? 1 : a; p = p == null ? 0.3 : p; if (x === 0 || x === 1) return x; const u = 2 * x - 1; const s = (p / (2 * Math.PI)) * Math.asin(1 / a); if (u < 0) return -0.5 * a * Math.pow(2, 10 * u) * Math.sin(((u - s) * 2 * Math.PI) / p); return 0.5 * a * Math.pow(2, -10 * u) * Math.sin(((u - s) * 2 * Math.PI) / p) + 1; };",
      matlab: "y = @(x, a, p) elasticInOutImpl(x, a, p);",
      excel:
        "=IF(A1=0,0,IF(A1=1,1,IF(2*A1-1<0,-0.5*B1*POWER(2,10*(2*A1-1))*SIN(((2*A1-1-(C1/(2*PI()))*ASIN(1/B1))*2*PI())/C1),0.5*B1*POWER(2,-10*(2*A1-1))*SIN(((2*A1-1-(C1/(2*PI()))*ASIN(1/B1))*2*PI())/C1)+1)))",
      desmos:
        "y=\\left(u<0\\right)\\left(-0.5a\\cdot2^{10u}\\sin\\left(\\frac{\\left(u-s\\right)2\\pi}{p}\\right)\\right)+\\left(u\\geq0\\right)\\left(0.5a\\cdot2^{-10u}\\sin\\left(\\frac{\\left(u-s\\right)2\\pi}{p}\\right)+1\\right)",
    },
    kernel,
