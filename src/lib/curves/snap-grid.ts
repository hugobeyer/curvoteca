...
export function snapGridCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SnapGrid(float x, float s = 0.1f) { s = s == 0 ? 0.1f : s; return MathF.Round(x / s) * s; }",
      ...
      gdscript: "func snapGrid(x: float, s: float = 0.1) -> float: if s == 0: s = 0.1; return round(x / s) * s",
