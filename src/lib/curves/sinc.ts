...
export function sincCurve(_params: SincParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Sinc(float x) { if (x == 0) return 1.0f; return MathF.Sin(MathF.PI * x) / (MathF.PI * x); }",
      ...
      python: "def sinc(x): return 1 if x == 0 else math.sin(math.pi * x) / (math.pi * x)",
      ...
      gdscript: "func sinc(x: float) -> float: if x == 0.0: return 1.0; return sin(PI * x) / (PI * x)",
