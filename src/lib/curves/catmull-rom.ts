...
export function catmullRomCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float CatmullRom(float x, float p0 = 0, float p1 = 0.5f, float p2 = 1, float p3 = 0.8f, float a = 0.5f) { float t0 = 0; float t1 = MathF.Pow(MathF.Abs(p1 - p0), a) + t0; float t2 = MathF.Pow(MathF.Abs(p2 - p1), a) + t1; float t3 = MathF.Pow(MathF.Abs(p3 - p2), a) + t2; float t = t1 + x * (t2 - t1); float A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; float B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; float C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; float D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; float E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E; }",
      ...
      python:
        "def catmull_rom(x, p0=0, p1=0.5, p2=1, p3=0.8, a=0.5): t0 = 0; t1 = abs(p1 - p0) ** a + t0; t2 = abs(p2 - p1) ** a + t1; t3 = abs(p3 - p2) ** a + t2; t = t1 + x * (t2 - t1); A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E",
      ...
      gdscript:
        "func catmullRom(x: float, p0: float = 0.0, p1: float = 0.5, p2: float = 1.0, p3: float = 0.8, a: float = 0.5) -> float: var t0 = 0.0; var t1 = pow(abs(p1 - p0), a) + t0; var t2 = pow(abs(p2 - p1), a) + t1; var t3 = pow(abs(p3 - p2), a) + t2; var t = t1 + x * (t2 - t1); var A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; var B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; var C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; var D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; var E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E",
      ...
      metal:
        "float catmullRom(float x, float p0, float p1, float p2, float p3, float a) { p0 = (p0 == 0.0) ? 0.0 : p0; p1 = (p1 == 0.0) ? 0.5 : p1; p2 = (p2 == 0.0) ? 1.0 : p2; p3 = (p3 == 0.0) ? 0.8 : p3; a = (a == 0.0) ? 0.5 : a; float t0 = 0.0; float t1 = pow(abs(p1 - p0), a) + t0; float t2 = pow(abs(p2 - p1), a) + t1; float t3 = pow(abs(p3 - p2), a) + t2; float t = t1 + x * (t2 - t1); float A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; float B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; float C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; float D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; float E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E; }",
      opencl:
        "float catmullRom(float x, float p0, float p1, float p2, float p3, float a) { float t0 = 0.0f; float t1 = pow(fabs(p1 - p0), a) + t0; float t2 = pow(fabs(p2 - p1), a) + t1; float t3 = pow(fabs(p3 - p2), a) + t2; float t = t1 + x * (t2 - t1); float A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; float B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; float C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; float D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; float E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E; }",
      unity:
        "public static float CatmullRom(float x, float p0 = 0, float p1 = 0.5f, float p2 = 1, float p3 = 0.8f, float a = 0.5f) { float t0 = 0; float t1 = MathF.Pow(MathF.Abs(p1 - p0), a) + t0; float t2 = MathF.Pow(MathF.Abs(p2 - p1), a) + t1; float t3 = MathF.Pow(MathF.Abs(p3 - p2), a) + t2; float t = t1 + x * (t2 - t1); float A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; float B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; float C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; float D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; float E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E; }",
      shadertoy:
        "float t0 = 0.0; float t1 = pow(abs(p1 - p0), a) + t0; float t2 = pow(abs(p2 - p1), a) + t1; float t3 = pow(abs(p3 - p2), a) + t2; float t = t1 + x * (t2 - t1); float A = (t1 - t) / (t1 - t0) * p0 + (t - t0) / (t1 - t0) * p1; float B = (t2 - t) / (t2 - t1) * p1 + (t - t1) / (t2 - t1) * p2; float C = (t3 - t) / (t3 - t2) * p2 + (t - t2) / (t3 - t2) * p3; float D = (t2 - t) / (t2 - t0) * A + (t - t0) / (t2 - t0) * B; float E = (t3 - t) / (t3 - t1) * B + (t - t1) / (t3 - t1) * C; return (t2 - t) / (t2 - t1) * D + (t - t1) / (t2 - t1) * E;",
      svelte:
        "export const catmullRom = (x, p0, p1, p2, p3, a) => catmullRomImpl(x, p0, p1, p2, p3, a);",
      matlab:
        "y = @(x, p0, p1, p2, p3, a) catmullRomImpl(x, p0, p1, p2, p3, a);",
    },
    kernel,
