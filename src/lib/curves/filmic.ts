...
export const defaultParams = { a: 0.22, b: 0.3, c: 0.1, d: 0.2, e: 0.01 } as const;
export type FilmicParams = { a: number; b: number; c: number; d: number; e: number };

export function filmicCurve(
  params: FilmicParams = defaultParams,
): CurveDefinition {
  const kernel: CurveKernel = filmicKernel(params.a, params.b, params.c, params.d, params.e);
  return {
    id: "filmic",
    ...
    snippets: {
      ...
      csharp: "float Filmic(float x, float a = 0.22f, float b = 0.3f, float c = 0.1f, float d = 0.2f, float e = 0.01f) { return x * (a * x + b) / (x * (c * x + d) + e); }",
      ...
      python: "def filmic(x, a=0.22, b=0.3, c=0.1, d=0.2, e=0.01): a = a or 0.22; b = b or 0.3; c = c or 0.1; d = d or 0.2; e = e or 0.01; return x * (a * x + b) / (x * (c * x + d) + e)",
      ...
      gdscript: "func filmic(x: float, a: float = 0.22, b: float = 0.3, c: float = 0.1, d: float = 0.2, e: float = 0.01) -> float: return x * (a * x + b) / (x * (c * x + d) + e)",
