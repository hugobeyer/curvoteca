...
export function bounceOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float BounceOut(float x, float n = 4, float k = 2) { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - MathF.Pow(1 - x, k) * (1 + MathF.Cos(2 * MathF.PI * n * x)) / 2; }",
      ...
      python:
        "def bounce_out(x, n=4, k=2): if x <= 0: return 0; if x >= 1: return 1; return 1 - (1 - x) ** k * (1 + math.cos(2 * math.pi * n * x)) / 2",
      ...
      gdscript:
        "func bounceOut(x: float, n: float = 4.0, k: float = 2.0) -> float: if x <= 0: return 0; if x >= 1: return 1; return 1 - pow(1 - x, k) * (1 + cos(2 * PI * n * x)) / 2",
      ...
      metal:
        "float bounceOut(float x, float n, float k) { n = (n < 1.0) ? 4.0 : n; k = (k == 0.0) ? 2.0 : k; if (x <= 0.0) return 0.0; if (x >= 1.0) return 1.0; return 1.0 - pow(1.0 - x, k) * (1.0 + cos(2.0 * 3.14159265 * n * x)) / 2.0; }",
      opencl:
        "float bounceOut(float x, float n, float k) { if (x <= 0.0f) return 0.0f; if (x >= 1.0f) return 1.0f; return 1.0f - pow(1.0f - x, k) * (1.0f + cos(2.0f * 3.14159265f * n * x)) / 2.0f; }",
      unity:
        "public static float BounceOut(float x, float n = 4, float k = 2) { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - MathF.Pow(1 - x, k) * (1 + MathF.Cos(2 * MathF.PI * n * x)) / 2; }",
      shadertoy:
        "if (x <= 0.0) return 0.0; if (x >= 1.0) return 1.0; return 1.0 - pow(1.0 - x, k) * (1.0 + cos(2.0 * 3.14159265 * n * x)) / 2.0;",
      svelte:
        "export const bounceOut = (x, n, k) => { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - Math.pow(1 - x, k) * (1 + Math.cos(2 * Math.PI * n * x)) / 2; };",
      matlab: "y = @(x, n, k) bounceOutImpl(x, n, k);",
    },
    kernel,
