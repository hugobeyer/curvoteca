...
export function expFalloffCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float ExpFalloff(float x, float k = 3f) { return (1.0f - MathF.Exp(-k * x)) / (1.0f - MathF.Exp(-k)); }",
      ...
      python: "def expFalloff(x, k=3): return (1 - math.exp(-k * x)) / (1 - math.exp(-k))",
      ...
      gdscript: "func exp_falloff(x: float, k: float = 3.0) -> float: return (1.0 - exp(-k * x)) / (1.0 - exp(-k))",
