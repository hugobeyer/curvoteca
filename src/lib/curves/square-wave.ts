...
export function squareWaveCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SquareWave(float x, float freq = 4) { return MathF.Sin(2 * MathF.PI * freq * x) < 0 ? -1 : 1; }",
      ...
      python:
        "def square_wave(x, freq=4): import math; return -1 if math.sin(2 * math.pi * freq * x) < 0 else 1",
      ...
      gdscript:
        "func squareWave(x: float, freq: float = 4.0) -> float: if freq < 0.01: freq = 4.0; return -1.0 if sin(2.0 * PI * freq * x) < 0 else 1.0",
      ...
      metal:
        "float squareWave(float x, float freq) { return sin(2.0 * 3.14159265 * freq * x) < 0.0 ? -1.0 : 1.0; }",
      opencl:
        "float squareWave(float x, float freq) { if (freq < 0.01f) freq = 4.0f; return sinf(2.0f * 3.14159265f * freq * x) < 0.0f ? -1.0f : 1.0f; }",
      unity:
        "public static float SquareWave(float x, float freq = 4) { return Mathf.Sin(2 * Mathf.PI * freq * x) < 0 ? -1 : 1; }",
      shadertoy: "return sin(2.0 * 3.14159265 * freq * x) < 0.0 ? -1.0 : 1.0;",
      matlab: "y = @(x, freq) sign(sin(2*pi*freq*x));",
    },
    kernel,
