...
export function dadsrCurve(
  ...
  return {
    ...
    domain: [0, 1],
    range: [0, 1],
    tags: ["envelope", "audio", "delay", "attack", "decay", "sustain", "release"],
    useCases: [
      "audio-envelope",
      "complex-shaping",
      "modulation",
    ...
    snippets: {
      ...
      csharp: "float Dadsr(float x, float d = 0.1f, float a = 0.2f, float de = 0.2f, float s = 0.3f, float r = 0.2f, float sl = 0.6f) { float t = x; if (t < d) return 0; t -= d; if (t < a) return t / a; t -= a; if (t < de) return 1 - (1 - sl) * t / de; t -= de; if (t < s) return sl; t -= s; return sl * (1 - t / r); }",
      ...
      python: "def dadsr(x, d=0.1, a=0.2, de=0.2, s=0.3, r=0.2, sl=0.6): t = x; if t < d: return 0; t -= d; if t < a: return t / a; t -= a; if t < de: return 1 - (1 - sl) * t / de; t -= de; if t < s: return sl; t -= s; return sl * (1 - t / r)",
      ...
      gdscript: "func dadsr(x: float, d: float = 0.1, a: float = 0.2, de: float = 0.2, s: float = 0.3, r: float = 0.2, sl: float = 0.6) -> float: var t = x; if t < d: return 0.0; t -= d; if t < a: return t / a; t -= a; if t < de: return 1.0 - (1.0 - sl) * t / de; t -= de; if t < s: return sl; t -= s; return sl * (1.0 - t / r)",
