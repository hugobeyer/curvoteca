...
export function bounceInCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float BounceIn(float x, float n = 4, float k = 2) { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - BounceOut(1 - x, n, k); }",
      ...
      python:
        "def bounce_in(x, n=4, k=2): if x <= 0: return 0; if x >= 1: return 1; return 1 - bounce_out(1 - x, n, k)",
      ...
      gdscript:
        "func bounceIn(x: float, n: float = 4.0, k: float = 2.0) -> float: if x <= 0: return 0; if x >= 1: return 1; return 1 - bounceOut(1 - x, n, k)",
      ...
      metal:
        "float bounceIn(float x, float n, float k) { n = (n < 1.0) ? 4.0 : n; k = (k == 0.0) ? 2.0 : k; if (x <= 0.0) return 0.0; if (x >= 1.0) return 1.0; return 1.0 - bounceOut(1.0 - x, n, k); }",
      opencl:
        "float bounceIn(float x, float n, float k) { if (x <= 0.0f) return 0.0f; if (x >= 1.0f) return 1.0f; return 1.0f - bounceOut(1.0f - x, n, k); }",
      unity:
        "public static float BounceIn(float x, float n = 4, float k = 2) { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - BounceOut(1 - x, n, k); }",
      shadertoy:
        "if (x <= 0.0) return 0.0; if (x >= 1.0) return 1.0; return 1.0 - bounceOut(1.0 - x, n, k);",
      svelte:
        "export const bounceIn = (x, n, k) => { if (x <= 0) return 0; if (x >= 1) return 1; return 1 - bounceOut(1 - x, n, k); };",
      matlab: "y = @(x, n, k) bounceInImpl(x, n, k);",
    },
    kernel,
