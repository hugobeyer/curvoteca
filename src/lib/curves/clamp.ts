...
export function clampCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float ClampFit(float x, float lo, float hi) { return x <= lo ? lo : (x >= hi ? hi : x); }",
      ...
      python: "def clamp_fit(x, lo, hi): return lo if x <= lo else (hi if x >= hi else x)",
      ...
      gdscript: "func clamp_fit(x: float, lo: float, hi: float) -> float: return lo if x <= lo else (hi if x >= hi else x)",
