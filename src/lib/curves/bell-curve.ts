...
export function bellCurveCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float BellCurve(float x, float mu = 0.5f, float sigma = 0.2f) { float z = (x - mu) / sigma; return MathF.Exp(-0.5f * z * z); }",
      ...
      python: "def bellCurve(x, mu=0.5, sigma=0.2): z = (x - mu) / sigma; return math.exp(-0.5 * z * z)",
      ...
      gdscript: "func bellCurve(x: float, mu: float = 0.5, sigma: float = 0.2) -> float: var z = (x - mu) / sigma; return exp(-0.5 * z * z)",
