...
export function adsrCurve(
  params: AdsrParams = defaultParams,
): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Adsr(float x, float a = 0.1f, float d = 0.2f, float s = 0.5f, float r = 0.3f, float sl = 0.6f) { if (x < a) return x / a; if (x < a + d) return 1 - (1 - sl) * (x - a) / d; if (x < a + d + s) return sl; if (x < 1) return sl * (1 - (x - a - d - s) / r); return 0; }",
      ...
      python: "def adsr(x, a=0.1, d=0.2, s=0.5, r=0.3, sl=0.6): a = a or 0.1; d = d or 0.2; s = s or 0.5; r = r or 0.3; sl = sl if sl >= 0.01 else 0.6; if x < a: return x / a; if x < a + d: return 1 - (1 - sl) * (x - a) / d; if x < a + d + s: return sl; if x < 1: return sl * (1 - (x - a - d - s) / r); return 0",
      ...
      gdscript: "func adsr(x: float, a: float = 0.1, d: float = 0.2, s: float = 0.5, r: float = 0.3, sl: float = 0.6) -> float: if x < a: return x / a; if x < a + d: return 1 - (1 - sl) * (x - a) / d; if x < a + d + s: return sl; if x < 1: return sl * (1 - (x - a - d - s) / r); return 0",
