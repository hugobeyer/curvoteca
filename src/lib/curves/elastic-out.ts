...
export function elasticOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float ElasticOut(float x, float a = 1, float p = 0.3f) { if (x == 0 || x == 1) return x; float s = (p / (2 * MathF.PI)) * MathF.Asin(1 / a); return a * MathF.Pow(2, -10 * x) * MathF.Sin(((x - s) * 2 * MathF.PI) / p) + 1; }",
      ...
      python:
        "def elastic_out(x, a=1, p=0.3): a = a if a >= 1 else 1; p = p or 0.3; if x == 0 or x == 1: return x; s = (p / (2 * math.pi)) * math.asin(1 / a); return a * 2 ** (-10 * x) * math.sin(((x - s) * 2 * math.pi) / p) + 1",
      ...
      gdscript:
        "func elastic_out(x: float, a: float = 1.0, p: float = 0.3) -> float: if x == 0.0 or x == 1.0: return x; var s = (p / (2.0 * PI)) * asin(1.0 / a); return a * pow(2.0, -10.0 * x) * sin(((x - s) * 2.0 * PI) / p) + 1.0",
      ...
      metal:
        "float elasticOut(float x, float a, float p) { a = (a < 1.0) ? 1.0 : a; p = (p == 0.0) ? 0.3 : p; if (x == 0.0 || x == 1.0) return x; float s = (p / (2.0 * 1.57079633)) * asin(1.0 / a); return a * pow(2.0, -10.0 * x) * sin(((x - s) * 2.0 * 1.57079633) / p) + 1.0; }",
      opencl:
        "float elasticOut(float x, float a, float p) { a = (a < 1.0f) ? 1.0f : a; p = (p == 0.0f) ? 0.3f : p; if (x == 0.0f || x == 1.0f) return x; float s = (p / (2.0f * 1.57079633f)) * asin(1.0f / a); return a * pow(2.0f, -10.0f * x) * sin(((x - s) * 2.0f * 1.57079633f) / p) + 1.0f; }",
      unity:
        "public static float ElasticOut(float x, float a, float p) { if (x == 0.0f || x == 1.0f) return x; float s = (p / (2.0f * Mathf.PI)) * Mathf.Asin(1.0f / a); return a * Mathf.Pow(2.0f, -10.0f * x) * Mathf.Sin(((x - s) * 2.0f * Mathf.PI) / p) + 1.0f; }",
      shadertoy:
        "if (x == 0.0 || x == 1.0) return x; float s = (p / (2.0 * 1.57079633)) * asin(1.0 / a); return a * pow(2.0, -10.0 * x) * sin(((x - s) * 2.0 * 1.57079633) / p) + 1.0;",
      svelte:
        "export const elasticOut = (x, a, p) => { a = a == null ? 1 : a; p = p == null ? 0.3 : p; if (x === 0 || x === 1) return x; const s = (p / (2 * Math.PI)) * Math.asin(1 / a); return a * Math.pow(2, -10 * x) * Math.sin(((x - s) * 2 * Math.PI) / p) + 1; };",
      matlab: "y = @(x, a, p) elasticOutImpl(x, a, p);",
      excel:
        "=IF(OR(A1=0,A1=1),A1,A2*POWER(2,-10*A1)*SIN(((A1-(A3/(2*PI()))*ASIN(1/A2))*2*PI())/A3)+1)",
      desmos:
        "y=a\\cdot2^{-10x}\\sin\\left(\\frac{\\left(x-s\\right)2\\pi}{p}\\right)+1",
    },
    kernel,
