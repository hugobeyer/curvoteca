...
export function sCurveContrastCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SCurveContrast(float x, float c = 1) { return x > 0.5f ? 1 - MathF.Pow(1 - x, 2 * c) : MathF.Pow(x, 2 * c); }",
      ...
      python:
        "def s_curve_contrast(x, c=1): c = c or 1; return 1 - math.pow(1 - x, 2 * c) if x > 0.5 else math.pow(x, 2 * c)",
      ...
      gdscript:
        "func sCurveContrast(x: float, c: float = 1.0) -> float: return 1.0 - pow(1.0 - x, 2.0 * c) if x > 0.5 else pow(x, 2.0 * c)",
      ...
      metal:
        "float sCurveContrast(float x, float c) { c = (c == 0.0) ? 1.0 : c; return x > 0.5 ? 1.0 - pow(1.0 - x, 2.0 * c) : pow(x, 2.0 * c); }",
      opencl:
        "float sCurveContrast(float x, float c) { return x > 0.5f ? 1.0f - pow(1.0f - x, 2.0f * c) : pow(x, 2.0f * c); }",
      unity:
        "public static float SCurveContrast(float x, float c = 1) { return x > 0.5f ? 1 - Mathf.Pow(1 - x, 2 * c) : Mathf.Pow(x, 2 * c); }",
    },
    kernel,
