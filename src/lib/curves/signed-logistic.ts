...
export function signedLogisticCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SignedLogistic(float x, float k) { return 2.0f / (1.0f + MathF.Exp(-k * x)) - 1.0f; }",
      ...
      python: "def signed_logistic(x, k): return 2 / (1 + math.exp(-k * x)) - 1",
      ...
      gdscript: "func signedLogistic(x: float, k: float) -> float: return 2.0 / (1.0 + exp(-k * x)) - 1.0",
