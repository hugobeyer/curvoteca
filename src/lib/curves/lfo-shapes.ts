...
export function lfoShapesCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float LfoShapes(float x, float s = 0, float f = 4) { float p = (x * f) % 1; float sine = MathF.Sin(2 * MathF.PI * p); float tri = 4 * Math.Abs(p - 0.5f) - 1; float saw = 2 * p - 1; float sq = p < 0.5f ? 1 : -1; float t = ((s % 1) + 1) % 1; return t < 0.25f ? sine : t < 0.5f ? tri : t < 0.75f ? saw : sq; }",
      ...
      python: "def lfo_shapes(x, s=0, f=4): f = f or 4; p = (x * f) % 1; sine = math.sin(2 * math.pi * p); tri = 4 * abs(p - 0.5) - 1; saw = 2 * p - 1; sq = 1 if p < 0.5 else -1; t = (s % 1 + 1) % 1; return sine if t < 0.25 else tri if t < 0.5 else saw if t < 0.75 else sq",
      ...
      gdscript: "func lfo_shapes(x: float, s: float = 0.0, f: float = 4.0) -> float: var p = fmod(x * f, 1.0); var sine = sin(2 * PI * p); var tri = 4 * abs(p - 0.5) - 1; var saw = 2 * p - 1; var sq = 1 if p < 0.5 else -1; var t = fmod(fmod(s, 1.0) + 1.0, 1.0); if t < 0.25: return sine; if t < 0.5: return tri; if t < 0.75: return saw; return sq",
