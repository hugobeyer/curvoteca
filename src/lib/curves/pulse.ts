...
export function pulseCurve(params: PulseParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Pulse(float x, float freq = 4) { float phase = (x * freq) % 1; return phase < 0.5 ? 1 : -1; }",
      ...
      python: "def pulse(x, freq=4): phase = (x * freq) % 1; return 1 if phase < 0.5 else -1",
      ...
      gdscript: "func pulse(x: float, freq: float = 4.0) -> float: var phase = fmod(x * freq, 1.0); return 1.0 if phase < 0.5 else -1.0",
