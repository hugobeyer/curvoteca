...
export function stepsEasingCurve(
  ...
  const kernel: CurveKernel = stepsEasingKernel(params.steps, params.smoothness);
  return {
    id: "steps-easing",
    name: "Steps Easing",
    ...
    snippets: {
      ...
      csharp: "float StepsEasing(float x, float n = 4, float s = 0) { n = Math.Max(1, n); s = Math.Max(0, s); float t = MathF.Floor(x * n) / n; if (s > 0) { float f = (x * n) % 1; float u = f / (s / n); u = Math.Clamp(u, 0, 1); t += (1.0f / n) * (u * u * (3 - 2 * u)); } return t; }",
      ...
      python: "def steps_easing(x, n=4, s=0): n = max(1, n); s = max(0, s); t = math.floor(x * n) / n; if s > 0: f = (x * n) % 1; u = max(0, min(1, f / (s / n))); t += (1 / n) * (u * u * (3 - 2 * u)); return t",
      css: "steps(n)",
      ...
      gdscript: "func stepsEasing(x: float, n: float = 4.0, s: float = 0.0) -> float: n = max(1.0, n); s = max(0.0, s); var t = floor(x * n) / n; if s > 0: var f = fmod(x * n, 1.0); var u = clamp(f / (s / n), 0.0, 1.0); t += (1.0 / n) * (u * u * (3.0 - 2.0 * u)); return t",
