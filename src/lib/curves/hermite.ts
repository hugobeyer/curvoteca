...
export function hermiteCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Hermite(float x, float p0 = 0, float p1 = 1, float m0 = 0, float m1 = 0) { float t2 = x * x; float t3 = t2 * x; return (2 * t3 - 3 * t2 + 1) * p0 + (t3 - 2 * t2 + x) * m0 + (-2 * t3 + 3 * t2) * p1 + (t3 - t2) * m1; }",
      ...
      python:
        "def hermite(x, p0=0, p1=1, m0=0, m1=0): t2 = x * x; t3 = t2 * x; return (2 * t3 - 3 * t2 + 1) * p0 + (t3 - 2 * t2 + x) * m0 + (-2 * t3 + 3 * t2) * p1 + (t3 - t2) * m1",
      ...
      gdscript:
        "func hermite(x: float, p0: float = 0.0, p1: float = 1.0, m0: float = 0.0, m1: float = 0.0) -> float: var t2 = x * x; var t3 = t2 * x; return (2 * t3 - 3 * t2 + 1) * p0 + (t3 - 2 * t2 + x) * m0 + (-2 * t3 + 3 * t2) * p1 + (t3 - t2) * m1",
      ...
      metal:
        "float hermite(float x, float p0, float p1, float m0, float m1) { float t2 = x * x; float t3 = t2 * x; return (2.0 * t3 - 3.0 * t2 + 1.0) * p0 + (t3 - 2.0 * t2 + x) * m0 + (-2.0 * t3 + 3.0 * t2) * p1 + (t3 - t2) * m1; }",
      opencl:
        "float hermite(float x, float p0, float p1, float m0, float m1) { float t2 = x * x; float t3 = t2 * x; return (2.0f * t3 - 3.0f * t2 + 1.0f) * p0 + (t3 - 2.0f * t2 + x) * m0 + (-2.0f * t3 + 3.0f * t2) * p1 + (t3 - t2) * m1; }",
      unity:
        "public static float Hermite(float x, float p0, float p1, float m0, float m1) { float t2 = x * x; float t3 = t2 * x; return (2.0f * t3 - 3.0f * t2 + 1.0f) * p0 + (t3 - 2.0f * t2 + x) * m0 + (-2.0f * t3 + 3.0f * t2) * p1 + (t3 - t2) * m1; }",
      matlab: "y = @(x, p0, p1, m0, m1) hermiteImpl(x, p0, p1, m0, m1);",
    },
    kernel,
