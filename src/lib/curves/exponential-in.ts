...
export function exponentialInCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float ExponentialIn(float x) { return x == 0.0f ? 0.0f : MathF.Pow(2.0f, 10.0f * x - 10.0f); }",
      ...
      python:
        "def exponential_in(x): return 0 if x == 0 else math.pow(2, 10 * x - 10)",
      ...
      gdscript:
        "func exponential_in(x: float) -> float: return 0.0 if x == 0.0 else pow(2.0, 10.0 * x - 10.0)",
      ...
      metal:
        "float exponentialIn(float x) { return x == 0.0 ? 0.0 : pow(2.0, 10.0 * x - 10.0); }",
      opencl:
        "float exponentialIn(float x) { return x == 0.0f ? 0.0f : pow(2.0f, 10.0f * x - 10.0f); }",
      unity:
        "public static float ExponentialIn(float x) { return x == 0.0f ? 0.0f : Mathf.Pow(2.0f, 10.0f * x - 10.0f); }",
      shadertoy: "return x == 0.0 ? 0.0 : pow(2.0, 10.0 * x - 10.0);",
      svelte:
        "export const exponentialIn = (x) => { return x === 0 ? 0 : Math.pow(2, 10 * x - 10); };",
      matlab: "y = @(x) exponentialInImpl(x);",
      excel: "=IF(A1=0,0,POWER(2,10*A1-10))",
      desmos:
        "y=\\left(x=0\\right)\\cdot0+\\left(x\\neq0\\right)\\cdot2^{10x-10}",
    },
    kernel,
