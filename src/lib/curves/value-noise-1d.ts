...
export function valueNoise1dCurve(
  ...
  return {
    ...
    snippets: {
      equation:
        "y = lerp(rand(floor(x)), rand(ceil(x)), smoothstep(0,1,frac(x)))",
      ...
      csharp:
        "float ValueNoise1d(float x, float seed = 1337) { /* hash + smoothstep */ return ValueNoise1dImpl(x, seed); }",
      ...
      python:
        "def value_noise_1d(x, seed=1337): # hash + smoothstep interpolation; return value_noise_1d_impl(x, seed)",
      ...
      gdscript:
        "func valueNoise1d(x: float, seed: float = 1337.0) -> float: # hash + smoothstep; return valueNoise1dImpl(x, seed)",
      ...
      metal:
        "float valueNoise1d(float x, float seed) { /* hash + smoothstep */ return valueNoise1dImpl(x, seed); }",
      opencl:
        "float valueNoise1d(float x, float seed) { /* hash + smoothstep */ return valueNoise1dImpl(x, seed); }",
      unity:
        "public static float ValueNoise1d(float x, float seed = 1337) { return ValueNoise1dImpl(x, seed); }",
      shadertoy: "return valueNoise1dImpl(x, seed);",
      matlab: "y = @(x, seed) valueNoise1dImpl(x, seed);",
    },
    kernel,
