...
export function circularOutCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float CircularOut(float x) { float u = 1 - x; return MathF.Sqrt(Math.Max(0.0f, 1 - u * u)); }",
      ...
      python: "def circular_out(x): u = 1 - x; return math.sqrt(max(0, 1 - u * u))",
      ...
      gdscript: "func circularOut(x: float) -> float: var u = 1 - x; return sqrt(max(0, 1 - u * u))",
