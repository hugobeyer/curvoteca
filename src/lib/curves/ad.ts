...
export function adCurve(
  params: AdParams = defaultParams,
): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Ad(float x, float a = 0.3f, float d = 0.4f) { return x < a ? x / a : 1 - (x - a) / d; }",
      ...
      python: "def ad(x, a=0.3, d=0.4): return x / a if x < a else 1 - (x - a) / d",
      ...
      gdscript: "func ad(x: float, a: float = 0.3, d: float = 0.4) -> float: return x / a if x < a else 1 - (x - a) / d",
