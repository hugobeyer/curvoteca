...
export function rubberBandCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float RubberBand(float x, float s = 3, float d = 0.5f) { if (x == 0) return 0; if (x >= 1) return 1; float o = s > 0 ? 1 : -1; return x + o * MathF.Exp(-d * x) * MathF.Sin(s * x); }",
      ...
      python:
        "def rubber_band(x, s=3, d=0.5): s = s or 3; d = d or 0.5; if x == 0: return 0; if x >= 1: return 1; o = 1 if s > 0 else -1; return x + o * math.exp(-d * x) * math.sin(s * x)",
      ...
      gdscript:
        "func rubberBand(x: float, s: float = 3.0, d: float = 0.5) -> float: if x == 0.0: return 0.0; if x >= 1.0: return 1.0; var o = 1.0 if s > 0.0 else -1.0; return x + o * exp(-d * x) * sin(s * x)",
      ...
      metal:
        "float rubberBand(float x, float s, float d) { s = (s == 0.0) ? 3.0 : s; d = (d == 0.0) ? 0.5 : d; if (x == 0.0) return 0.0; if (x >= 1.0) return 1.0; float o = s > 0.0 ? 1.0 : -1.0; return x + o * exp(-d * x) * sin(s * x); }",
      opencl:
        "float rubberBand(float x, float s, float d) { if (x == 0.0f) return 0.0f; if (x >= 1.0f) return 1.0f; float o = s > 0.0f ? 1.0f : -1.0f; return x + o * exp(-d * x) * sin(s * x); }",
      unity:
        "public static float RubberBand(float x, float s = 3, float d = 0.5f) { if (x == 0) return 0; if (x >= 1) return 1; float o = s > 0 ? 1 : -1; return x + o * Mathf.Exp(-d * x) * Mathf.Sin(s * x); }",
    },
    kernel,
