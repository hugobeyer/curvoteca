import { DEFAULT_SAMPLING, fbmKernel, type CurveKernel } from "../curveMath";
...
export function fbm1dCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Fbm1d(float x, float seed = 1337, float octaves = 4, float lacunarity = 2, float gain = 0.5f) { /* summed value-noise octaves */ return Fbm1dImpl(x, seed, octaves, lacunarity, gain); }",
      ...
      python:
        "def fbm_1d(x, seed=1337, octaves=4, lacunarity=2, gain=0.5): # summed value-noise octaves; return fbm_1d_impl(x, seed, octaves, lacunarity, gain)",
      ...
      gdscript:
        "func fbm_1d(x: float, seed: float = 1337.0, octaves: float = 4.0, lacunarity: float = 2.0, gain: float = 0.5) -> float: # summed value-noise octaves; return fbm_1d_impl(x, seed, octaves, lacunarity, gain)",
      ...
      metal:
        "float fbm1d(float x, float seed, float octaves, float lacunarity, float gain) { /* summed value-noise octaves */ return fbm1dImpl(x, seed, octaves, lacunarity, gain); }",
      opencl:
        "float fbm1d(float x, float seed, float octaves, float lacunarity, float gain) { /* summed value-noise octaves */ return fbm1dImpl(x, seed, octaves, lacunarity, gain); }",
      unity:
        "public static float Fbm1d(float x, float seed, float octaves, float lacunarity, float gain) { /* summed value-noise octaves */ return Fbm1dImpl(x, seed, octaves, lacunarity, gain); }",
      shadertoy: "return fbm1dImpl(x, seed, octaves, lacunarity, gain);",
      matlab:
        "y = @(x, seed, octaves, lacunarity, gain) fbm_1d_impl(x, seed, octaves, lacunarity, gain);",
    },
    kernel,
