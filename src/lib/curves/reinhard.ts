import { DEFAULT_SAMPLING, reinhardKernel, type CurveKernel } from "../curveMath";
...
export function reinhardCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float Reinhard(float x, float wp = 1) { return x * (1 + x / (wp * wp)) / (1 + x); }",
      ...
      python: "def reinhard(x, wp=1): wp = wp or 1; return x * (1 + x / (wp * wp)) / (1 + x)",
      ...
      gdscript: "func reinhard(x: float, wp: float = 1.0) -> float: return x * (1 + x / (wp * wp)) / (1 + x)",
