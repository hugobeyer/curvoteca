...
export function compressorCurve(
  ...
  const kernel: CurveKernel = compressorKernel(
    params.threshold,
    params.ratio,
    params.knee,
  );
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Compressor(float x, float t = 0.5f, float r = 4, float k = 0.1f) { if (x < t - k) return x; if (x > t + k) return t + (x - t) / r; float u = (x - t + k) / (2 * k); return x + (u * u * (3 - 2 * u)) * ((t + (x - t) / r) - x); }",
      ...
      python:
        "def compressor(x, t=0.5, r=4, k=0.1): t = t or 0.5; r = r or 4; k = k or 0.1; if x < t - k: return x; if x > t + k: return t + (x - t) / r; u = (x - t + k) / (2 * k); return x + (u * u * (3 - 2 * u)) * ((t + (x - t) / r) - x)",
      ...
      gdscript:
        "func compressor(x: float, t: float = 0.5, r: float = 4.0, k: float = 0.1) -> float: if x < t - k: return x; if x > t + k: return t + (x - t) / r; var u = (x - t + k) / (2.0 * k); return x + (u * u * (3.0 - 2.0 * u)) * ((t + (x - t) / r) - x)",
      ...
      metal:
        "float compressor(float x, float t, float r, float k) { t = (t == 0.0) ? 0.5 : t; r = (r == 0.0) ? 4.0 : r; k = (k == 0.0) ? 0.1 : k; if (x < t - k) return x; if (x > t + k) return t + (x - t) / r; float u = (x - t + k) / (2.0 * k); return x + (u * u * (3.0 - 2.0 * u)) * ((t + (x - t) / r) - x); }",
      opencl:
        "float compressor(float x, float t, float r, float k) { t = (t == 0.0) ? 0.5f : t; r = (r == 0.0) ? 4.0f : r; k = (k == 0.0) ? 0.1f : k; if (x < t - k) return x; if (x > t + k) return t + (x - t) / r; float u = (x - t + k) / (2.0f * k); return x + (u * u * (3.0f - 2.0f * u)) * ((t + (x - t) / r) - x); }",
      unity:
        "public static float Compressor(float x, float t = 0.5f, float r = 4, float k = 0.1f) { if (x < t - k) return x; if (x > t + k) return t + (x - t) / r; float u = (x - t + k) / (2 * k); return x + (u * u * (3 - 2 * u)) * ((t + (x - t) / r) - x); }",
    },
    kernel,
