...
export function expCurve(params: ExpParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float ExpCurve(float x, float k = 2f) { return (MathF.Exp(k * x) - 1.0f) / (MathF.Exp(k) - 1.0f); }",
      ...
      python: "def expCurve(x, k=2): return (math.exp(k * x) - 1) / (math.exp(k) - 1)",
      ...
      gdscript: "func exp_curve(x: float, k: float = 2.0) -> float: return (exp(k * x) - 1.0) / (exp(k) - 1.0)",
