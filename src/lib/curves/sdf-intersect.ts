...
export function sdfIntersectCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SdfIntersect(float x, float r1 = 0.3f, float c1 = 0.3f, float r2 = 0.3f, float c2 = 0.7f) { float d1 = MathF.Abs(x - c1) - r1; float d2 = MathF.Abs(x - c2) - r2; return MathF.Max(d1, d2); }",
      ...
      python:
        "def sdf_intersect(x, r1=0.3, c1=0.3, r2=0.3, c2=0.7): d1 = abs(x - c1) - r1; d2 = abs(x - c2) - r2; return max(d1, d2)",
      ...
      gdscript:
        "func sdfIntersect(x: float, r1: float = 0.3, c1: float = 0.3, r2: float = 0.3, c2: float = 0.7) -> float: var d1 = abs(x - c1) - r1; var d2 = abs(x - c2) - r2; return max(d1, d2)",
      ...
      metal:
        "float sdfIntersect(float x, float r1, float c1, float r2, float c2) { r1 = (r1 == 0.0) ? 0.3 : r1; c1 = (c1 == 0.0) ? 0.3 : c1; r2 = (r2 == 0.0) ? 0.3 : r2; c2 = (c2 == 0.0) ? 0.7 : c2; float d1 = abs(x - c1) - r1; float d2 = abs(x - c2) - r2; return max(d1, d2); }",
      opencl:
        "float sdfIntersect(float x, float r1, float c1, float r2, float c2) { float d1 = fabs(x - c1) - r1; float d2 = fabs(x - c2) - r2; return fmax(d1, d2); }",
      unity:
        "public static float SdfIntersect(float x, float r1 = 0.3f, float c1 = 0.3f, float r2 = 0.3f, float c2 = 0.7f) { float d1 = Mathf.Abs(x - c1) - r1; float d2 = Mathf.Abs(x - c2) - r2; return Mathf.Max(d1, d2); }",
    },
    kernel,
