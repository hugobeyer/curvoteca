...
export function exponentialInOutCurve(
  ...
  return {
    ...
    formula:
      "y = 0 if x=0; (2^(20x-10))/2 if x<0.5; 1 if x=1; (2 - 2^(-20x+10))/2 else",
    continuity: "C3+",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      ...
      csharp:
        "float ExponentialInOut(float x) { if (x == 0.0f) return 0.0f; if (x == 1.0f) return 1.0f; return x < 0.5f ? MathF.Pow(2.0f, 20.0f * x - 10.0f) / 2.0f : (2.0f - MathF.Pow(2.0f, -20.0f * x + 10.0f)) / 2.0f; }",
      ...
      python:
        "def exponential_in_out(x): if x == 0: return 0; if x == 1: return 1; return math.pow(2, 20 * x - 10) / 2 if x < 0.5 else (2 - math.pow(2, -20 * x + 10)) / 2",
      ...
      gdscript:
        "func exponential_in_out(x: float) -> float: if x == 0.0: return 0.0; if x == 1.0: return 1.0; return pow(2.0, 20.0 * x - 10.0) / 2.0 if x < 0.5 else (2.0 - pow(2.0, -20.0 * x + 10.0)) / 2.0",
      ...
      metal:
        "float exponentialInOut(float x) { if (x == 0.0) return 0.0; if (x == 1.0) return 1.0; return x < 0.5 ? pow(2.0, 20.0 * x - 10.0) / 2.0 : (2.0 - pow(2.0, -20.0 * x + 10.0)) / 2.0; }",
      opencl:
        "float exponentialInOut(float x) { if (x == 0.0f) return 0.0f; if (x == 1.0f) return 1.0f; return x < 0.5f ? pow(2.0f, 20.0f * x - 10.0f) / 2.0f : (2.0f - pow(2.0f, -20.0f * x + 10.0f)) / 2.0f; }",
      unity:
        "public static float ExponentialInOut(float x) { if (x == 0.0f) return 0.0f; if (x == 1.0f) return 1.0f; return x < 0.5f ? Mathf.Pow(2.0f, 20.0f * x - 10.0f) / 2.0f : (2.0f - Mathf.Pow(2.0f, -20.0f * x + 10.0f)) / 2.0f; }",
      shadertoy:
        "if (x == 0.0) return 0.0; if (x == 1.0) return 1.0; return x < 0.5 ? pow(2.0, 20.0 * x - 10.0) / 2.0 : (2.0 - pow(2.0, -20.0 * x + 10.0)) / 2.0;",
      svelte:
        "export const exponentialInOut = (x) => { if (x === 0) return 0; if (x === 1) return 1; return x < 0.5 ? Math.pow(2, 20 * x - 10) / 2 : (2 - Math.pow(2, -20 * x + 10)) / 2; };",
      matlab: "y = @(x) exponentialInOutImpl(x);",
      excel:
        "=IF(A1=0,0,IF(A1=1,1,IF(A1<0.5,POWER(2,20*A1-10)/2,(2-POWER(2,-20*A1+10))/2)))",
      desmos:
        "y=\\left(x=0\\right)\\cdot0+\\left(x=1\\right)\\cdot1+\\left(x<0.5\\right)\\cdot\\frac{2^{20x-10}}{2}+\\left(x>0.5\\right)\\cdot\\frac{2-2^{-20x+10}}{2}",
    },
    kernel,
    ...
    factory: {
      ...
      produce: (params) =>
        exponentialInOutCurve(params as ExponentialInOutParams),
    },
  };
}
