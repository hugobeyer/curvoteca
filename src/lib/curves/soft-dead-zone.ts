...
export function softDeadZoneCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp: "float SoftDeadZone(float x, float d = 0.2f) { float a = MathF.Abs(x); if (a < d) return 0.0f; float u = (a - d) / (1.0f - d); float y = u * u * (3.0f - 2.0f * u); return x < 0 ? -y : y; }",
      ...
      python: "def softDeadZone(x, d=0.2): a = abs(x); return 0 if a < d else (lambda u: u * u * (3 - 2 * u))((a - d) / (1 - d)) * (-1 if x < 0 else 1)",
      ...
      gdscript: "func softDeadZone(x: float, d: float = 0.2) -> float: var a = abs(x); if a < d: return 0.0; var u = (a - d) / (1.0 - d); var y = u * u * (3.0 - 2.0 * u); return -y if x < 0 else y",
