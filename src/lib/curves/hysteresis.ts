...
export function hysteresisCurve(
  ...
  const kernel: CurveKernel = hysteresisKernel(
    params.threshold,
    params.width,
  );
  return {
    ...
    snippets: {
      ...
      csharp: "float Hysteresis(float x, float t = 0.3f, float w = 0.1f) { if (x < t - w) return 0; if (x > t + w) return 1; return (x - (t - w)) / (2 * w); }",
      ...
      python: "def hysteresis(x, t=0.3, w=0.1): if x < t - w: return 0; if x > t + w: return 1; return (x - (t - w)) / (2 * w)",
      ...
      gdscript: "func hysteresis(x: float, t: float = 0.3, w: float = 0.1) -> float: if x < t - w: return 0; if x > t + w: return 1; return (x - (t - w)) / (2 * w)",
