import { DEFAULT_SAMPLING, asrKernel, type CurveKernel } from "../curveMath";
...
export function asrCurve(params: AsrParams = defaultParams): CurveDefinition {
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float Asr(float x, float a = 0.2f, float s = 0.5f, float r = 0.3f, float sl = 0.7f) { if (x < a) return x / a; if (x < a + s) return sl; return sl * (1 - (x - a - s) / r); }",
      ...
      python:
        "def asr(x, a=0.2, s=0.5, r=0.3, sl=0.7): if x < a: return x / a; if x < a + s: return sl; return sl * (1 - (x - a - s) / r)",
      ...
      gdscript:
        "func asr(x: float, a: float = 0.2, s: float = 0.5, r: float = 0.3, sl: float = 0.7) -> float: if x < a: return x / a; if x < a + s: return sl; return sl * (1 - (x - a - s) / r)",
      ...
      metal:
        "float asr(float x, float a, float s, float r, float sl) { a = (a < 0.01) ? 0.2 : a; s = (s < 0.01) ? 0.5 : s; r = (r < 0.01) ? 0.3 : r; if (x < a) return x / a; if (x < a + s) return sl; return sl * (1.0 - (x - a - s) / r); }",
      opencl:
        "float asr(float x, float a, float s, float r, float sl) { if (x < a) return x / a; if (x < a + s) return sl; return sl * (1.0f - (x - a - s) / r); }",
      unity:
        "public static float Asr(float x, float a = 0.2f, float s = 0.5f, float r = 0.3f, float sl = 0.7f) { if (x < a) return x / a; if (x < a + s) return sl; return sl * (1 - (x - a - s) / r); }",
    },
    kernel,
