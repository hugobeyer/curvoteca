...
export function simplexNoise1dCurve(
  ...
  return {
    ...
    snippets: {
      ...
      csharp:
        "float SimplexNoise1d(float x, float seed = 1337) { /* skew + kernel + grad */ return Simplex1dImpl(x, seed); }",
      ...
      python:
        "def simplex_noise_1d(x, seed=1337): # skew + kernel + grad; return simplex_1d_impl(x, seed)",
      ...
      gdscript:
        "func simplexNoise1d(x: float, seed: float = 1337.0) -> float: # mulberry32 + skewed 1D simplex; return simplex1dImpl(x, seed)",
      ...
      metal:
        "float simplexNoise1d(float x, float seed) { /* skew + kernel + grad */ return simplex1dImpl(x, seed); }",
      opencl:
        "float simplexNoise1d(float x, float seed) { /* mulberry32 + skewed 1D simplex */ return simplex1dImpl(x, seed); }",
      unity:
        "public static float SimplexNoise1d(float x, float seed = 1337) { /* skew + kernel + grad */ return Simplex1dImpl(x, seed); }",
      shadertoy: "return simplex1dImpl(x, seed);",
      matlab: "y = @(x, seed) simplex_1d_impl(x, seed);",
    },
    kernel,
    sampling: DEFAULT_SAMPLING,
    related: ["perlin-noise-1d", "value-noise-1d", "fbm-1d", "worley-noise-1d"],
    factory: {
