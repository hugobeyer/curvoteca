import { DEFAULT_SAMPLING, springKernel, type CurveKernel } from "../curveMath";
...
export function springCurve(
  ...
  return {
    ...
    formula: "y = 1 - e^(-ζωt) * (cos(ω_d*t) + (ζω/ω_d)*sin(ω_d*t))",
    continuity: "C1",
    domain: [0, 1],
    range: [0, 1],
    ...
    snippets: {
      equation: "y = 1 - e^(-ζωt) * (cos(ω_d*t) + (ζω/ω_d)*sin(ω_d*t))",
      ...
      csharp:
        "float Spring(float x, float d = 0.5f, float m = 1, float s = 4) { if (x == 0) return 0; if (x >= 1) return 1; float omega = MathF.Sqrt(s / m); float zeta = d / (2 * MathF.Sqrt(s * m)); float wd = omega * MathF.Sqrt(1 - zeta * zeta); float e = MathF.Exp(-zeta * omega * x); return 1 - e * (MathF.Cos(wd * x) + (zeta * omega / wd) * MathF.Sin(wd * x)); }",
      ...
      python:
        "def spring(x, d=0.5, m=1, s=4): d = d if d >= 0.01 else 0.5; m = m or 1; s = s or 4; if x == 0: return 0; if x >= 1: return 1; omega = math.sqrt(s / m); zeta = d / (2 * math.sqrt(s * m)); wd = omega * math.sqrt(1 - zeta * zeta); e = math.exp(-zeta * omega * x); return 1 - e * (math.cos(wd * x) + (zeta * omega / wd) * math.sin(wd * x))",
      ...
      gdscript:
        "func spring(x: float, d: float = 0.5, m: float = 1.0, s: float = 4.0) -> float: if x == 0: return 0.0; if x >= 1: return 1.0; var omega = sqrt(s / m); var zeta = d / (2 * sqrt(s * m)); var wd = omega * sqrt(1 - zeta * zeta); var e = exp(-zeta * omega * x); return 1 - e * (cos(wd * x) + (zeta * omega / wd) * sin(wd * x))",
      ...
      metal:
        "float spring(float x, float d, float m, float s) { d = (d < 0.01) ? 0.5 : d; m = (m == 0.0) ? 1.0 : m; s = (s == 0.0) ? 4.0 : s; if (x == 0.0) return 0.0; if (x >= 1.0) return 1.0; float omega = sqrt(s / m); float zeta = d / (2.0 * sqrt(s * m)); float wd = omega * sqrt(1.0 - zeta * zeta); float e = exp(-zeta * omega * x); return 1.0 - e * (cos(wd * x) + (zeta * omega / wd) * sin(wd * x)); }",
      opencl:
        "float spring(float x, float d, float m, float s) { if (d < 0.01f) d = 0.5f; if (m == 0.0f) m = 1.0f; if (s == 0.0f) s = 4.0f; if (x == 0.0f) return 0.0f; if (x >= 1.0f) return 1.0f; float omega = sqrt(s / m); float zeta = d / (2.0f * sqrt(s * m)); float wd = omega * sqrt(1.0f - zeta * zeta); float e = exp(-zeta * omega * x); return 1.0f - e * (cos(wd * x) + (zeta * omega / wd) * sin(wd * x)); }",
      unity:
        "public static float Spring(float x, float d = 0.5f, float m = 1, float s = 4) { if (x == 0) return 0; if (x >= 1) return 1; float omega = Mathf.Sqrt(s / m); float zeta = d / (2 * Mathf.Sqrt(s * m)); float wd = omega * Mathf.Sqrt(1 - zeta * zeta); float e = Mathf.Exp(-zeta * omega * x); return 1 - e * (Mathf.Cos(wd * x) + (zeta * omega / wd) * Mathf.Sin(wd * x)); }",
    },
    kernel,
